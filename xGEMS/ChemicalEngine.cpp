...
namespace xGEMS
{
    ...
    {
        if(pimpl->node->check_IC_xCH(ielement)) {
            return pimpl->node->pCSD()->ICNL[ielement];
        }
        return {};
    }
    ...
    auto ChemicalEngine::options() const -> const ChemicalEngineOptions&
    {
    ...
    auto operator<<(std::ostream &out, const ChemicalEngine &state) -> std::ostream &
    {
        ...
        const ChemicalEngineOptions &options = state.options();
        ...
        // Build element table for filtered phases only
        std::vector<std::string> element_names;
        std::vector<Vector> element_rows;
        for (Index i = 0; i < state.numElements(); ++i)
        {
            Vector row(valid_phase_indices.size());
            for (size_t j = 0; j < valid_phase_indices.size(); ++j)
            {
        ...
        // Filter phase state properties values accordingly
        std::vector<Vector> values1;
        auto filter_vector = [&](const Vector &v) -> Vector
        {
            Vector filtered(valid_phase_indices.size());
            for (size_t i = 0; i < valid_phase_indices.size(); ++i)
                filtered[i] = v[valid_phase_indices[i]];
            return filtered;
        };

        values1.push_back(filter_vector(state.phaseAmounts()));
        values1.push_back(filter_vector(state.phaseMasses()));
        values1.push_back(filter_vector(state.phaseVolumes()));
        values1.push_back(filter_vector(state.phaseDensities()));
        values1.push_back(filter_vector(state.phaseSatIndices()));
        ...
        std::vector<Vector> species_rows;

        for (Index i = 0; i < state.numSpecies(); ++i)
        {
            // Filter criterion: skip species with zero amount
            if (n[i] == 0.0 && !options.print_zero_amounts)
                continue;
            Vector row(property_names.size());
            row[0] = n[i];
            row[1] = concentrations[i];
            row[2] = activity_coeffs[i];
            row[3] = molfractions[i];
        ...
        printWrappedTable(out, "Species", property_names, species_names, species_rows);
